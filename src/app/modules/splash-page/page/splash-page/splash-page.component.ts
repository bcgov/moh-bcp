...
export class SplashPageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.sub$ = this.splashService.values.subscribe(splashVals => {
      if (splashVals) {
        this.startTime = splashVals.SPA_ENV_BCP_MAINTENANCE_START;
        this.endTime = splashVals.SPA_ENV_BCP_MAINTENANCE_END;
        this.message = splashVals.SPA_ENV_BCP_MAINTENANCE_MESSAGE;
      }
